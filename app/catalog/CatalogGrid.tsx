          ...
          {products.map((p) => (
            <div key={p.id} className="rounded-lg border border-slate-200 p-4">
              <Link href={`/products/${p.slug}`} className="block">
                ...
                <h3 className="font-semibold hover:text-brand-blue">{p.name}</h3>
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="font-semibold text-brand-blue">{formatNaira(p.price)}</span>
                  <span className={`text-xs ${p.inStock ? "text-green-600" : "text-amber-600"}`}>
